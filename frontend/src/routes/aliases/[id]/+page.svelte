<script lang="ts">
  import { onMount } from 'svelte';
  import { page } from '$app/stores';
  import { writable } from 'svelte/store';
  import { aliasApi } from '$lib/api/aliases';
  import { apiClient } from '$lib/api/client';
  import type { JsonApiResource, Alias, Image } from '$lib/types';
  
  let aliasId = '';
  let alias = null;
  let recentImages = [];
  const loadingStore = writable(true);
  const errorStore = writable('');
  const imageCount = writable(0);

  onMount(async () => {
    aliasId = $page.params.id;
    
    try {
      // Load alias details
      alias = await aliasApi.getById(aliasId);
      
      // Load recent images for this alias
      const imagesResponse = await apiClient.get(`/api/v1/aliases/${aliasId}/images?per_page=12`);
      recentImages = imagesResponse.data;
      imageCount.set(imagesResponse.meta.total_count);
      
      loadingStore.set(false);
    } catch (err) {
      errorStore.set(err.message || 'Failed to load alias details');
      loadingStore.set(false);
    }
  });
</script>

<svelte:head>
  <title>{alias?.attributes.name || 'Artist'} - CustomBooru</title>
</svelte:head>

<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
  <div class="px-4 py-6 sm:px-0">
    {#if $loadingStore}
      <div class="flex justify-center items-center py-12">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>
      </div>
    {:else if $errorStore}
      <div class="rounded-md bg-red-50 p-4">
        <div class="text-sm text-red-700">
          {$errorStore}
        </div>
      </div>
    {:else if alias}
      <!-- Breadcrumb -->
      <nav class="flex mb-8" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          <li class="inline-flex items-center">
            <a href="/aliases" class="text-gray-500 hover:text-gray-700">
              Artists
            </a>
          </li>
          <li>
            <div class="flex items-center">
              <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path>
              </svg>
              <span class="text-gray-900 ml-1 md:ml-2 font-medium">
                {alias.attributes.name}
              </span>
            </div>
          </li>
        </ol>
      </nav>

      <!-- Artist Profile -->
      <div class="bg-white shadow rounded-lg overflow-hidden mb-8">
        <div class="px-6 py-8">
          <div class="flex items-start space-x-6">
            <!-- Avatar -->
            <div class="flex-shrink-0">
              <div class="w-20 h-20 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white text-2xl font-bold">
                {alias.attributes.name.charAt(0).toUpperCase()}
              </div>
            </div>
            
            <!-- Info -->
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between">
                <div>
                  <h1 class="text-2xl font-bold text-gray-900">
                    {alias.attributes.name}
                  </h1>
                  <p class="text-sm text-gray-500 mt-1">
                    {$imageCount} image{$imageCount !== 1 ? 's' : ''}
                  </p>
                </div>
                
                <!-- Actions -->
                <div class="flex space-x-3">
                  <a
                    href="/aliases/{aliasId}/images/upload"
                    class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg transition-colors"
                  >
                    Upload Image
                  </a>
                  <a
                    href="/aliases/{aliasId}/images"
                    class="bg-gray-600 hover:bg-gray-700 text-white font-medium py-2 px-4 rounded-lg transition-colors"
                  >
                    View All Images
                  </a>
                </div>
              </div>
              
              <!-- Bio -->
              {#if alias.attributes.bio}
                <div class="mt-4">
                  <p class="text-gray-700 whitespace-pre-wrap">{alias.attributes.bio}</p>
                </div>
              {/if}
              
              <!-- Social Links -->
              {#if alias.attributes.social_links && Object.keys(alias.attributes.social_links).length > 0}
                <div class="mt-4">
                  <h3 class="text-sm font-medium text-gray-900 mb-2">Social Links</h3>
                  <div class="flex flex-wrap gap-2">
                    {#each Object.entries(alias.attributes.social_links) as [platform, url]}
                      <a
                        href={url}
                        target="_blank"
                        rel="noopener noreferrer"
                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 transition-colors"
                      >
                        {platform}
                        <svg class="ml-1 w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                        </svg>
                      </a>
                    {/each}
                  </div>
                </div>
              {/if}
            </div>
          </div>
        </div>
      </div>

      <!-- Recent Images -->
      <div class="bg-white shadow rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
          <div class="flex items-center justify-between">
            <h2 class="text-lg font-medium text-gray-900">Recent Images</h2>
            {#if recentImages.length > 0}
              <a
                href="/aliases/{aliasId}/images"
                class="text-sm text-indigo-600 hover:text-indigo-500 font-medium"
              >
                View all {$imageCount} images →
              </a>
            {/if}
          </div>
        </div>
        
        <div class="p-6">
          {#if recentImages.length === 0}
            <div class="text-center py-12">
              <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <h3 class="mt-2 text-sm font-medium text-gray-900">No images yet</h3>
              <p class="mt-1 text-sm text-gray-500">Get started by uploading the first image for this artist.</p>
              <div class="mt-6">
                <a
                  href="/aliases/{aliasId}/images/upload"
                  class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 transition-colors"
                >
                  <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd" />
                  </svg>
                  Upload Image
                </a>
              </div>
            </div>
          {:else}
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {#each recentImages as image (image.id)}
                <div class="bg-white overflow-hidden shadow rounded-lg group hover:shadow-md transition-shadow">
                  <div class="aspect-square bg-gray-200 overflow-hidden">
                    {#if image.attributes.file_url}
                      <img
                        src={image.attributes.file_url}
                        alt={image.attributes.title}
                        class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-200"
                      />
                    {:else}
                      <div class="w-full h-full flex items-center justify-center">
                        <svg class="h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                      </div>
                    {/if}
                  </div>
                  
                  <div class="p-4">
                    <h3 class="text-sm font-medium text-gray-900 truncate">
                      {image.attributes.title || 'Untitled'}
                    </h3>
                    
                    {#if image.attributes.tags_list && image.attributes.tags_list.length > 0}
                      <div class="mt-2 flex flex-wrap gap-1">
                        {#each image.attributes.tags_list.slice(0, 3) as tag}
                          <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">
                            {tag}
                          </span>
                        {/each}
                        {#if image.attributes.tags_list.length > 3}
                          <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">
                            +{image.attributes.tags_list.length - 3} more
                          </span>
                        {/if}
                      </div>
                    {/if}

                    <div class="mt-4 flex justify-between items-center text-xs text-gray-500">
                      <span>
                        {new Date(image.attributes.created_at).toLocaleDateString()}
                      </span>
                      <a
                        href="/aliases/{aliasId}/images/{image.id}"
                        class="text-indigo-600 hover:text-indigo-500 font-medium"
                      >
                        View
                      </a>
                    </div>
                  </div>
                </div>
              {/each}
            </div>
          {/if}
        </div>
      </div>
    {/if}
  </div>
</div>